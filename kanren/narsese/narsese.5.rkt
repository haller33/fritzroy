...
(define (string-index str ch)
  (let ((len (string-length str)))
    (let loop ((i 0))
      (if (>= i len) #f
          (if (char=? (string-ref str i) ch) i (loop (+ i 1)))))))

(define (string-contains-char? str ch)
  (not (not (string-index str ch))))
...
(define (parse-var type str)
  (make-var type (string->symbol str)))
...
(define (parse-statement str)
  (let ((trimmed (string-trim str)))
    (cond ((string-prefix? trimmed "<") (parse-relation trimmed))
          ...
          ((string-prefix? trimmed "#") (parse-var 'universal trimmed))
          ((string-prefix? trimmed "?") (parse-var 'query trimmed))
          ((string-prefix? trimmed "$") (parse-var 'existential trimmed))
...
;; parse-composite function with this enhanced version:
(define (parse-composite str)
  ;; str is like "( ... )" where ... is the content
  ;; Handles:
  ;;   (&&, A, B, C)   -> conjunction
  ;;   (||, A, B, C)   -> disjunction
  ;;   (--, A)         -> negation
  ;;   (&/, A, B, C)   -> sequential conjunction (simplified to conjunction)
  ;;   (* A B) or (A * B) -> product
  ;;   (∩ A B) or (A ∩ B) -> intersection
  ;;   (∪ A B) or (A ∪ B) -> union
  (let* ((content (substring str 1 (- (string-length str) 1)))
         (parts (split-on-commas content)))
    (cond
      ;; Prefix operators: first element is operator like "&&", "||", "--", "&/"
      ((and (pair? parts) (member (car parts) '("&&" "||" "--" "&/")))
       (let ((op (string->symbol (car parts)))
         ...
         (cond ((eq? op '&&) (make-conj args))
               ...
               (else (error "Unknown prefix operator")))))
      ;; Infix product (A * B)
      ((string-contains-char? content #\*)
       (let ((pos (string-index content #\*)))
         (if pos
             (let ((left (string-trim (substring content 0 pos)))
                   (right (string-trim (substring content (+ pos 1)))))
               (make-prod (list (parse-statement left) (parse-statement right))))
             (error "Invalid product"))))
      ;; Infix intersection (A ∩ B)
      ((string-contains-char? content #\∩)
       (let ((pos (string-index content #\∩)))
         (if pos
             (let ((left (string-trim (substring content 0 pos)))
                   (right (string-trim (substring content (+ pos 1)))))
               (make-intersection (list (parse-statement left) (parse-statement right))))
      ...
      ((string-contains-char? content #\∪)
       (let ((pos (string-index content #\∪)))
         (if pos
             (let ((left (string-trim (substring content 0 pos)))
                   (right (string-trim (substring content (+ pos 1)))))
               (make-union (list (parse-statement left) (parse-statement right))))
             (error "Invalid union"))))
      (else (error (format "Unknown composite format: ~a" str))))))


(define (parse-composite-broken str)
  ;; Handles:
  ;;   (&&, A, B, C)   -> conjunction
  ;;   (||, A, B, C)   -> disjunction
  ;;   (--, A)         -> negation
  ...
  (let* ((content (substring str 1 (- (string-length str) 1)))
         (parts (split-on-commas content)))
    (cond
      ;; Prefix operators (first element is operator)
      ((and (pair? parts)
            (member (car parts) '("&&" "||" "--" "&/" "*" "∩" "∪")))
       (let ((op (string->symbol (car parts)))
             (args (map parse-statement (cdr parts))))
         (cond ((eq? op '&&) (make-conj args))
               ((eq? op '||) (make-disj args))
               ((eq? op '--) (if (= (length args) 1) (make-neg (car args)) (error "-- needs one argument")))
               ((eq? op '&/) (make-conj args))   ; sequential conjunction
               ((eq? op '*) (make-prod args))
               ((eq? op '∩) (make-intersection args))
               ((eq? op '∪) (make-union args))
               (else (error "Unknown operator" op)))))
      ;; Infix product: (A * B) where "*" is not the first element
      ((and (pair? parts) (member "*" parts))
       (let ((star-pos (list-index (lambda (x) (equal? x "*")) parts)))
         (if star-pos
             (let ((left (parse-statement (string-join (take parts star-pos) " ")))
                   (right (parse-statement (string-join (drop parts (+ star-pos 1)) " "))))
               (make-prod (list left right)))
             (error "Invalid product")))
       )
      ;; Infix intersection: (A ∩ B)
      ((and (pair? parts) (member "∩" parts))
       (let ((pos (list-index (lambda (x) (equal? x "∩")) parts)))
         (if pos
             (let ((left (parse-statement (string-join (take parts pos) " ")))
                   (right (parse-statement (string-join (drop parts (+ pos 1)) " "))))
               (make-intersection (list left right)))
             (error "Invalid intersection")))
       )
      ;; Infix union: (A ∪ B)
      ((and (pair? parts) (member "∪" parts))
       (let ((pos (list-index (lambda (x) (equal? x "∪")) parts)))
         (if pos
             (let ((left (parse-statement (string-join (take parts pos) " ")))
                   (right (parse-statement (string-join (drop parts (+ pos 1)) " "))))
               (make-union (list left right)))
             (error "Invalid union")))
       )
      (else (error "Unknown composite format")))
    ))

(define (parse-composite-old str)
  (let* ((content (substring str 1 (- (string-length str) 1)))
         (parts (split-on-commas content)))
    (let ((op (string->symbol (car parts)))
          (args (map parse-statement (cdr parts))))
